	...
	def learnNetwork( self, max_iter=50, method='autoshift' ):
		...
		def callback_elt_param(x_i):
			...
			if self.iter_num % 5 == 0:
				os.makedirs('adj_recon/', exist_ok=True)
...
def main():
	...
	parser.add_argument('-f', '--filename', required=False,default="sbm1.mat", help="Relative path to dataset file")
	...
	parser.add_argument('-r', '--rank', required=False, default="8", help="Rank of approximation for embedding (default is 128)")
	...
	adjacency = N.getAdjacency()
	embeddings = N.get_LR_embedding()
	# Learn Adjacency Matrix having same embedding
	adjacency2_npy = np.load("adjacency1.npy")
	adjacency2 = sparse.csr_matrix(adjacency2_npy)
	embeddings_npy = np.load("embedding.npy")
	w, v = np.linalg.eigh(embeddings_npy)
	order = np.argsort(np.abs(w))[::-1]
	w, v = w[order[:rank]], v[:, order[:rank]]
	LR_Embedding = v @ np.diag(w) @ v.T
	embeddings1 = np.asmatrix(LR_Embedding)
	P = Optimizer( adjacency2, embeddings1, args.filename, args.it, args.rank )
	P.learnNetwork() 
